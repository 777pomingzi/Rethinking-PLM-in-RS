...
class RecentModelLogger(AbstractBaseLogger):
    def __init__(self, checkpoint_path,args, filename='checkpoint-recent.pth'):
...
class BestModelLogger(AbstractBaseLogger):
    def __init__(self, checkpoint_path, args,metric_key='mean_iou', filename='best_acc_model.pth'):
        self.checkpoint_path = checkpoint_path
        if args.local_rank==0 and not os.path.exists(self.checkpoint_path):
            os.mkdir(self.checkpoint_path)
