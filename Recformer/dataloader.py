...
class RecformerEvalDataset(Dataset):
    def __init__(self, user2train, user2val, user2test, user2neg, mode, collator: EvalDataCollatorWithPadding):
        self.user2train = user2train
        self.user2val = user2val
        self.user2test = user2test
        self.collator = collator
        self.user2neg=user2neg
    ...
    def __getitem__(self, index):
        ...
        if self.user2neg is not None:
            negs =  self.user2neg[user]
            return seq, label, label+negs
        else:
            return seq, label, None

    def collate_fn(self, data):

        return self.collator([{'items': line[0], 'label': line[1], 'candidates':line[2]} for line in data])
