...
class RecformerForSeqRec(LongformerPreTrainedModel):
    ...
    def similarity_score(self, pooler_output, candidates=None):
        if candidates is None:
            ...
            candidate_embeddings = self.item_embedding(candidates-1) # (batch_size, candidates, hidden_size)
    ...
    def forward(self,
        ...
        if self.config.finetune_negative_sample_size<=0: ## using full softmax
            ...
            loss = loss_fct(logits, labels-1)

        else:  ## using sampled softmax
            candidates = torch.cat((labels.unsqueeze(-1), torch.randint(1, self.config.item_num+1, size=(batch_size, self.config.finetune_negative_sample_size)).to(labels.device)), dim=-1)
